...
def combine_similarities(embed_sims: dict, struct_sims: dict, alpha=0.6):
    """
    Combine embedding similarities and structural heuristics.
    alpha = weight for embeddings, (1-alpha) = weight for structural heuristics.
    """
    combined = {}
    for k in embed_sims.keys():
        e = float(embed_sims.get(k, 0.0))
        s = float(struct_sims.get(k, 0.0))
        combined[k] = alpha * e + (1.0 - alpha) * s
    # optional renormalize to 0..1
    vals = list(combined.values())
    min_v = min(vals)
    max_v = max(vals)
    if max_v - min_v > 1e-9:
        for k in combined:
            combined[k] = (combined[k] - min_v) / (max_v - min_v)
    return combined
...
def post_snapshot(payload: SnapshotPayload):
    ...
    embed_sims = compute_similarities(emb)

    # compute structural heuristics and combine
    struct_sims = structural_scores(code)    # from code above
    combined = combine_similarities(embed_sims, struct_sims, alpha=0.6)  # tune alpha as needed
    ...
    obs = {
        ...
        "embed_similarities": embed_sims,
        "struct_similarities": struct_sims,
        "similarities": combined,   # the one used downstream
    }
    with LOCK:
        SESSION_STORE[session].append(obs)
    return {"ok": True, "session": session, "snapshot_id": payload.snapshot_id, "similarities": combined,
            "debug": {"embed": embed_sims, "struct": struct_sims}}
