...
int main() {
  ...
  const runCode = async () => {
     try {
      const res = await axios.post('http://127.0.0.1:5000/api/run_code', {
        language,
        code,
  ...
  output: tc.output,       // store actual output
  status: tc.status        // pass/fail/tle/error
})));
      }
  ...
  <CardContent>
    ...
    {testcases.map((tc) => (
      ...
      >
        ...
        {tc.visible && (
          <AccordionDetails>
            {/* only the details section scrolls if long */}
      ...
      {/* AI Interview Panel */}
      <div className="w-1/3 border-l flex flex-col p-4">
        <Typography variant="h6">AI Interviewer</Typography>
        <div className="flex-1 overflow-auto my-2 space-y-2">
          {chat.map((msg, i) => (
            <div
              key={i}
